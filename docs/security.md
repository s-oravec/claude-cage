...
When using SLIRP networking (the default), claude-cage adds blackhole routes inside the VM via cloud-init:
...
When using bridge networking (`--network bridge`), claude-cage applies iptables rules on the host:
...
Network isolation is enabled by default. The blocked subnets can be configured in `~/.claude-cage/config.yaml`:
